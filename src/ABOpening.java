...
public class ABOpening {
    ...
    public static void main(String[] args) {
        try {
            ...
            char[] output = game.MaxMin(board, depth, Integer.MIN_VALUE, Integer.MAX_VALUE);
        ...
        for (int i = 0; i < board.length; i++) {
            if (board[i] == x) {
                ...
                if (closeMill(i, copy)) {
                    ...
                    System.out.println("3 in a row, W removed");
                } else {
    ...
    public ArrayList<String> generateBlackMoves(char[] tempB) {
        ...
        for (int i=0;i<boardConfigs.size();i++) {
            char[] b = boardConfigs.get(i).toCharArray();
            boardConfigs.set(i, new String(swapPiecePositions(b)));
        }
        System.out.println(boardConfigs);
    ...
    public char[] MaxMin(char[] board, int depth, int alpha, int beta) {
        if (depth > 0) {
            depth--;
            List<String> positions = generateBlackMoves(board);
            char[] minB;
            ...
            for (String child : positions) {
                ...
                minB = MinMax(child.toCharArray(), depth, alpha, beta);
    ...
    public char[] MinMax(char[] board, int depth, int alpha, int beta) {
        if (depth > 0) {
            depth--;
            List<String> positions = generateAdd(board);
            ...
            for (String child : positions) {
                System.out.println(child);
                maxB = MaxMin(child.toCharArray(), depth, alpha, beta);
